...
def reduce_n_features_1d(input_layer, net_setup, block_name, l1_first_layer_reg = 0):
    prev_layer = input_layer
    layer_sizes = get_layer_size_sequence(net_setup)
    for n, layer_size in enumerate(layer_sizes):
        if n == 0 and l1_first_layer_reg >0:  
            setup = copy.deepcopy(net_setup)
            setup.kernel_regularizer = tf.keras.regularizers.L1(l1=l1_first_layer_reg)
        else: 
            setup = net_setup
