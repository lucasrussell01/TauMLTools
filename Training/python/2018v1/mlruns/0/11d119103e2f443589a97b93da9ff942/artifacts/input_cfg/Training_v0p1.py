...
class NetSetup:
    def __init__(self, activation, dropout_rate=0, reduction_rate=1, kernel_regularizer=None):
        ...
        if self.activation == 'relu' or self.activation == 'PReLU' or self.activation == 'tanh' or self.activation == "sigmoid" or self.activation == "softplus":
...
def dense_block(prev_layer, kernel_size, net_setup, block_name, n, use_bias = True):
...
def reduce_n_features_1d(input_layer, net_setup, block_name, first_layer_reg = None):
    ...
    for n, layer_size in enumerate(layer_sizes):
        if n == 0 and first_layer_reg is not None:  # regularisation
...
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run(**run_kwargs) as active_run:
        ...
        #LR try printing tau weights
        print("Input_tau weights:\n")
        print((model.get_layer("input_tau").weights))
