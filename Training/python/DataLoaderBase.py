...
class GetData():
            ...
            _X.append(
                torch.cat(
                    [ __class__.getdata(_obj_grid[ getattr(R.CellObjectType,fname) ][_inner],
                     (batch_size, _n_cells, _n_cells, n_grid_features[fname]), area=fname + "_at_getgrid") for fname in group ],
                    dim=-1
                    )
                )
        return _X
    ...
    def getsequence(_obj_grid,
        ...
        return [ __class__.getdata(_obj_grid[getattr(R.CellObjectType,group)],
                (_n_tau, _n_seq[group], _n_features[group]), area=fname + "_at_getsequence")
    ...
    def getX(data,
        ...
        if 'TauFlat' in active_features:
            X_all += [ __class__.getdata(data.x_tau, (batch_size, n_flat_features), area="TauFlat") ]
        # Inner grid
