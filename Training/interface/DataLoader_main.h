...
public:
        ...
        hasData(false), fullData(false), hasFile(false)
    {
      ...
      // ***** LR: Embedded data major changes START 1/2 ***** 
      ...
      MaxDisbCheck(hist_weights, weight_thr); // LR: Check embedded weight imbalance
      std::cout << "Embedded Histogram Processed \n";
      // ***** LR: Embedded data major changes END 1/2 ***** 
    }

    DataLoader(const DataLoader&) = delete;
    DataLoader& operator=(const DataLoader&) = delete;

    void ReadFile(std::string file_name, Long64_t start_file, Long64_t end_file) { 
        tauTuple.reset();
    ...
    bool MoveNext() {
        ...
        // ***** LR: Embedded data major changes START 2/2 ***** 
        while(tau_i < n_tau) { // LR: While batch not full
          if(tau_i < n_embtau) { // LR: If there are not the maximum number of embedded taus
            ...
            if (gen_match && tau.tau_pt <500){
              ...
              // LR: skip event if it is not tau_h fro binary classification (02/22)
              if(tau.tauType == 2) {
            ...
            if (!tau_is_set && include_mismatched)
              ++tau_i;
            ++current_entry;
          } // ***** LR: Embedded data major changes END 2/2 ***** 
        }
        fullData = true;
        return true;
    }
...
private:
  ...
  std::unique_ptr<TauTuple> embtauTuple; // LR: Declare embedded tau tuple
